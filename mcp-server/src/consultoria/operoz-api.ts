...
/** Wrapper fino para as chamadas à API do Operoz usadas pela consultoria. */
export class ConsultoriaApi {
  ...
  private async v1<T>(
    ...
    try {
      ...
      if (error instanceof OperozApiError && error.status === 429 && attempt < MAX_RETRIES) {
...
export function getWsSlug(args: Record<string, unknown>): string {
  ...
  const fromEnv = process.env.OPEROZ_WORKSPACE_SLUG?.trim() ?? "";
  const slug = fromArgs || fromEnv;
  if (!slug) {
    throw new Error(
      "workspace_slug é obrigatório. Defina OPEROZ_WORKSPACE_SLUG no ambiente ou passe workspace_slug na chamada."
    );
  }
  return slug;
}
