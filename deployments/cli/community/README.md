...
Installing operoz is a very easy and minimal step process.
...
Above steps will set you ready to install and start operoz services.
...
This will create a folder `operoz-app` and will download 2 files inside that
...
This will create a create a folder `operoz-app` and will download 2 files inside that
...
> `WEB_URL` - This is default set to `http://localhost`. Change this to the FQDN you plan to use along with LISTEN_HTTP_PORT (eg. `https://operoz.example.com:8080` or `http://[IP-ADDRESS]:8080`)

> `CORS_ALLOWED_ORIGINS` - This is default set to `http://localhost`. Change this to the FQDN you plan to use along with LISTEN_HTTP_PORT (eg. `https://operoz.example.com:8080` or `http://[IP-ADDRESS]:8080`)
...
You have successfully self hosted `Operoz` instance. Access the application by going to IP or domain you have configured it (e.g `https://operoz.example.com:8080` or `http://[IP-ADDRESS]:8080`)

---

### Stopping the Server / Remove Stack

In case you want to make changes to `operoz.env` variables, we suggest you to stop the services before doing that.
...
In case you want to make changes to `operoz.env` variables, without stopping the server or you noticed some abnormalies in services, you can restart the services with `RESTART` / `REDEPLOY` option.
...
### Upgrading Operoz Version

It is always advised to keep Operoz up to date with the latest release.
...
By choosing this, it will stop the services and then will download the latest `docker-compose.yaml` and `operoz.env`.
...
> It is very important for you to validate the `operoz.env` for the new changes.

Once done with making changes in `operoz.env` file, jump on to `Start Server`
...
Once done with making changes in `operoz.env` file, jump on to `Redeploy Stack`
...
```bash
Backing Up operoz-app_pgdata
Backing Up operoz-app_redisdata
Backing Up operoz-app_uploads

Backup completed successfully. Backup files are stored in /....../operoz-app/backup/20240502-1120
```
...
1. Make sure that Operoz-CE is installed, started, and then stopped. This ensures that the Docker volumes are created.
...
1. Execute the command below to restore your data.
   ...
   As an example, for a backup folder `/opt/operoz-selfhost/operoz-app/backup/20240722-0914`, expect the response below:
   ...
   --------------------------------------------
   Found /opt/operoz-selfhost/operoz-app/backup/20240722-0914/pgdata.tar.gz
   .....Restoring operoz-app_pgdata
   .....Successfully restored volume operoz-app_pgdata from pgdata.tar.gz

   Found /opt/operoz-selfhost/operoz-app/backup/20240722-0914/redisdata.tar.gz
   .....Restoring operoz-app_redisdata
   .....Successfully restored volume operoz-app_redisdata from redisdata.tar.gz

   Found /opt/operoz-selfhost/operoz-app/backup/20240722-0914/uploads.tar.gz
   .....Restoring operoz-app_uploads
   .....Successfully restored volume operoz-app_uploads from uploads.tar.gz


   Restore completed successfully.
   ```

1. Start the Operoz instance using `./setup.sh start`.

---

### Restore for Commercial Air-Gapped (Docker Compose)

When you want to restore the previously backed-up data on Operoz Commercial Air-Gapped version, follow the instructions below.
...
1. Make sure that Operoz Commercial (Airgapped) is extracted and ready to get started. In case it is running, you would need to stop that.
...
1. After restoration, you are ready to start Operoz Commercial (Airgapped) will all your previously saved data.
...
To move forward, you would need PREFIX of old setup and new setup. As per above example, `v0132` is the prefix of v0.13.2 and `operoz-app` is the prefix of v0.14.0 setup
...
Provide the Destination Volume Prefix : operoz-app
```
