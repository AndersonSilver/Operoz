...
function restoreData() {
    ...
    dockerServiceStatus=$($COMPOSE_CMD ls --filter name=operoz-app --format=json | jq -r .[0].Status)
    local dockerServicePrefix
    dockerServicePrefix="running"

    if [[ $dockerServiceStatus == $dockerServicePrefix* ]]; then
        echo "Operoz App is running. Please STOP the Operoz App before restoring data."
        exit 1
    fi
    ...
    local volumes
    volumes=$(docker volume ls -f "name=operoz-app" --format "{{.Name}}" | grep -E "$volume_suffix")
    # Check if there are any matching volumes
    if [ -z "$volumes" ]; then
        echo ".....No volumes found starting with 'operoz-app'"
        exit 1
    fi
    ...
    for BACKUP_FILE in $BACKUP_FOLDER/*.tar.gz; do
        if [ -e "$BACKUP_FILE" ]; then
            ...
            restoreVolName="operoz-app_${restoreFileName}"
