...
Antes de implementar **qualquer** UI, ler e seguir a skill mestre **`OPEROZ ENGENHARIA/SKILL.md`** (loop JTBD → estados → checklist). Não saltar mesmo em fixes pequenos.

Resumo não negociável:

- Pensar como produto: mental model, progressive disclosure, copy pt-BR Operoz.
- URL/token/UUID → copiar + toast (`IconButton` + `Tooltip`), nunca bloco `break-all`.
- Modal mantém contexto após sucesso (ex.: multi-convite PRD review).
- Tokens + telas irmãs: `DESIGN SISTEMA/` + `operoz-frontend-design.mdc`.
- PRD review guest: UX alinhada com `harness-engineering/prd-review-client-upload.md`.
...
| Pasta                                | Uso                                        |
| ------------------------------------ | ------------------------------------------ |
| `OPEROZ ENGENHARIA/SKILL.md`         | **UX produto** — loop antes de qualquer UI |
| `OPEROZ FLUXO/CONTEXTO`              | Domínio, Git, Harness, custos              |
| `OPEROZ ENGENHARIA/DESIGN SISTEMA`   | Tokens e consistência cross-tela           |
