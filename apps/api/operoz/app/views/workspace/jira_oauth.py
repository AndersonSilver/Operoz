...
from operoz.utils.jira_ops.oauth import (
...
class WorkspaceJiraOpsOAuthCompleteEndpoint(BaseAPIView):
    ...
    def post(self, request, slug):
        ...
        if pending:
            ...
            for resource in pending.get("resources", []):
                if resource.get("id") == cloud_id:
                    config.jira_site_name = resource.get("name") or ""
                    break
            from operoz.utils.jira_ops.oauth import pop_pending_clouds
        ...
        try:
            from operoz.utils.jira_ops.oauth import ensure_fresh_oauth_token
