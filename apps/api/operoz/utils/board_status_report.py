...
from operoz.db.models import Issue, Module, Project
...
if TYPE_CHECKING:
    from operoz.db.models import Board, User
...
def apply_live_entregas_from_module(report, content: dict, user: User) -> dict:
    ...
    if report_kind in ("sprint", "multi_module") or (
        ...
        rr = sections.setdefault("report_row", {})
        rr["fim"] = entrega_date_label(max_module_target_date(modules))
        return content
...
def build_project_status_report_content(
    *,
    ...
    primary = modules[0]
    sprint_end_date = max_module_target_date(modules)
    consultor_name = user_consultor_label(user)
    responsavel_cliente = project_responsavel_cliente_label(project)
    base = _build_module_report_metrics(
        project=project,
    ...
    sections = {
        **base,
        ...
        "report_row": {
            ...
            "fim": entrega_date_label(sprint_end_date),
        },
