"""Importa Jira OPS → Operoz: cliente=projeto, épico=módulo, card=issue, subtarefa=sub-issue."""
...
from operoz.utils.project_default_features import ensure_project_default_features
...
JIRA_MODULE_STATUS_TO_OPEROZ: dict[str, str] = {
...
def resolve_state(project: Project, fields: dict) -> State | None:
    ...
    state_name = JIRA_STATE_TO_OPEROZ.get(status_name, "Todo")
...
def resolve_module_status(fields: dict) -> str:
    """Map Jira epic (Projeto) status → Operoz module status."""
    status = fields.get("status") or {}
    status_name = _normalize_jira_status_name(status.get("name") or "")
    mapped = JIRA_MODULE_STATUS_TO_OPEROZ.get(status_name)
    if mapped:
        return mapped
