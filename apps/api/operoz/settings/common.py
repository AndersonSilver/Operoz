...
# Module imports
from operoz.utils.url import is_valid_url
...
WEBHOOK_ALLOWED_IPS = []
_logger = logging.getLogger("operoz")
...
MIDDLEWARE = [
    ...
    "operoz.authentication.middleware.session.SessionMiddleware",
    ...
    "operoz.middleware.request_body_size.RequestBodySizeLimitMiddleware",
    "operoz.middleware.logger.APITokenLogMiddleware",
    "operoz.middleware.logger.RequestLoggerMiddleware",
]
...
REST_FRAMEWORK = {
    ...
    "EXCEPTION_HANDLER": "operoz.authentication.adapter.exception.auth_exception_handler",
...
WSGI_APPLICATION = "operoz.wsgi.application"
ASGI_APPLICATION = "operoz.asgi.application"
...
if os.environ.get("ENABLE_READ_REPLICA", "0") == "1":
    ...
    # Database Routers
    DATABASE_ROUTERS = ["operoz.utils.core.dbrouters.ReadReplicaRouter"]
    # Add middleware at the end for read replica routing
    MIDDLEWARE.append("operoz.middleware.db_routing.ReadReplicaRoutingMiddleware")
...
STORAGES["default"] = {"BACKEND": "operoz.settings.storage.S3Storage"}
...
CELERY_TASK_ROUTES = {
    "operoz.bgtasks.automation_task.run_board_automation": {"queue": AUTOMATION_CELERY_QUEUE},
    "operoz.bgtasks.automation_task.enqueue_automation_outbox": {"queue": AUTOMATION_CELERY_QUEUE},
    "operoz.bgtasks.automation_task.flush_stale_automation_outbox": {"queue": AUTOMATION_CELERY_QUEUE},
...
CELERY_IMPORTS = (
    ...
    # issue version tasks
    "operoz.bgtasks.issue_version_sync",
    "operoz.bgtasks.issue_description_version_sync",
)
...
# Jira OPS → Operoz (fallback legado; preferir configuração por workspace)
