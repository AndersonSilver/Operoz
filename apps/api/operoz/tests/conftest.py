...
def _patch_pgvector_test_db() -> None:
    ...
    if getattr(DatabaseCreation, "_operoz_pgvector_patched", False):
        return
    ...
    DatabaseCreation._operoz_pgvector_patched = True
...
def session_client(api_client, create_user):
    """Return a session authenticated API client for app API testing, which is what operoz.app uses"""
