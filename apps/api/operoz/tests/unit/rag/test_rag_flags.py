...
from operoz.rag.indexing_scheduler import schedule_entity_index
from operoz.rag.rag_flags import is_rag_indexing_enabled
...
class TestRagIndexingFlag:
    ...
    def test_disabled_by_falsy_values(self, value):
        with override_settings(RAG_INDEXING_ENABLED=value):
...
class TestSchedulerRespectsFlag:
    @override_settings(RAG_INDEXING_ENABLED="0")
    def test_schedule_does_not_enqueue_when_disabled(self, monkeypatch):
        calls = []
        monkeypatch.setattr(
            "operoz.rag.indexing_scheduler.mark_index_pending",
            lambda *args, **kwargs: calls.append("mark"),
        )
        monkeypatch.setattr(
            "operoz.rag.indexing_scheduler.index_entity_task.apply_async",
...
class TestIndexEntityRespectsFlag:
    ...
    def test_index_entity_skips_without_touching_db(self, db, monkeypatch):
        ...
        monkeypatch.setattr("operoz.rag.indexing._load_entity_chunks", _fail)
