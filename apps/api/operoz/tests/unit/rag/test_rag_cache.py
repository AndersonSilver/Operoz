...
from operoz.rag.rag_cache import get_cached_rag_results, normalize_query, query_hash, store_rag_results
...
class TestRagCache:
    ...
    @patch("operoz.rag.rag_cache.cache")
...
class TestQueryEmbeddingCache:
    @patch("operoz.rag.embedding_cache.embedding_cache_scope", return_value="openai:text-embedding-3-small")
    @patch("operoz.rag.embedding_cache._embed_texts_uncached")
    @patch("operoz.rag.embedding_cache.cache")
