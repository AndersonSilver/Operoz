...
from operoz.utils.status_report_export import max_module_target_date
...
@pytest.mark.django_db
def test_sprint_report_row_fim_uses_max_module_target_date(
    workspace,
    workspace_board,
    report_project,
    create_user,
...
def test_max_module_target_date_returns_latest():
    ...
    modules = [
        StubModule(date(2026, 5, 8)),
        StubModule(date(2026, 8, 24)),
        StubModule(None),
    ]
    assert max_module_target_date(modules) == date(2026, 8, 24)
    assert max_module_target_date([StubModule(None)]) is None
