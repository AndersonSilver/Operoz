...
from operoz.automation.email_renderer import deliver_automation_email, send_automation_email
...
class TestAutomationEmailQueue:
    ...
    @patch("operoz.bgtasks.automation_email_task.send_automation_email_task.delay")
    ...
    @patch("operoz.automation.email_renderer.EmailMultiAlternatives")
    @patch("operoz.automation.email_renderer.get_instance_smtp_connection")
    def test_deliver_sends_smtp(self, mock_smtp, mock_email_cls):
        mock_smtp.return_value = (MagicMock(), "noreply@operoz.local")
        msg = MagicMock()
