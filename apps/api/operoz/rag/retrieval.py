...
from operoz.rag.embeddings import embed_texts
from operoz.rag.embedding_cache import embed_query_cached
from operoz.rag.rag_cache import get_cached_rag_results, store_rag_results
from operoz.rag.security.access import accessible_projects, can_access_page, get_accessible_issue
from operoz.rag.types import AssistantActorContext
from operoz.db.models import Page, SearchEmbedding
...
def _rag_top_k() -> int:
    return int(getattr(settings, "RAG_TOP_K", 5))


def _rag_candidate_limit() -> int:
    return int(getattr(settings, "RAG_CANDIDATE_LIMIT", 30))
...
def is_rag_enabled() -> bool:
    return str(getattr(settings, "RAG_ENABLED", "1")).lower() not in ("0", "false", "no")
...
def _rag_hnsw_ef_search() -> int:
    return int(getattr(settings, "RAG_HNSW_EF_SEARCH", 40))
