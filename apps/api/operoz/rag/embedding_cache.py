...
from operoz.rag.rag_cache import normalize_query
...
def _content_cache_ttl() -> int:
    raw = rag_env("EMBEDDING_CACHE_TTL", str(DEFAULT_TTL_SECONDS))
    try:
...
def _query_cache_ttl() -> int:
    raw = rag_env("QUERY_EMBEDDING_CACHE_TTL", str(DEFAULT_QUERY_TTL_SECONDS))
    try:
        return max(60, int(raw))
    except ValueError:
