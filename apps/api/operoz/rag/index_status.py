...
from operoz.rag.embeddings import content_hash
from operoz.rag.page_content import build_page_indexable_text
from operoz.rag.retrieval import is_rag_enabled
...
def resolve_page_index_status(page: Page) -> dict[str, Any]:
    ...
    from operoz.rag.indexing_scheduler import ensure_page_index_queued
...
def _public_status_payload(
    ...
    *,
    ...
    status = record.get("status", "not_indexed")
    message_keys = {
        "disabled": "rag.index.disabled",
        "empty": "rag.index.empty",
        "not_indexed": "rag.index.not_indexed",
        "pending": "rag.index.pending",
        "processing": "rag.index.processing",
        "indexed": "rag.index.indexed",
        "failed": "rag.index.failed",
        "stale": "rag.index.stale",
    }
    payload = {
        "status": status,
        ...
        "message_key": message_keys.get(status, "rag.index.not_indexed"),
