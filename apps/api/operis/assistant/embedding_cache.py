...
from operis.assistant.embeddings import _embed_texts_uncached, content_hash, get_embedding_config
from operis.assistant.rag_cache import normalize_query
...
def embed_texts_cached(texts: Sequence[str]) -> list[list[float]] | None:
    """Embeddings com cache por hash de conteúdo (Redis via Django cache)."""
    if not texts:
        return []

    _, model = get_embedding_config()
    ...
    for index, (text, digest) in enumerate(zip(cleaned, hashes, strict=True)):
        if not text:
            vectors[index] = []
            continue
        cached = _load_cached_vector(CACHE_PREFIX, model, digest)
    ...
    if misses:
        fetched = _embed_texts_uncached([text for _, text in misses])
        if fetched is None:
            return None
        for (index, _), vector in zip(misses, fetched, strict=True):
            vectors[index] = vector
            if vector:
                _store_cached_vector(CACHE_PREFIX, model, hashes[index], vector, ttl=_content_cache_ttl())
...
def embed_query_cached(query: str) -> list[float] | None:
    ...
    cached = _load_cached_vector(QUERY_CACHE_PREFIX, model, digest)
    if cached is not None:
        from operis.assistant.observability import record_rag_cache_access

        record_rag_cache_access(hit=True)
        logger.debug("assistant query embedding cache hit model=%s digest=%s", model, digest[:12])
        return cached

    from operis.assistant.observability import record_rag_cache_access

    record_rag_cache_access(hit=False)
    fetched = _embed_texts_uncached([normalized])
    if not fetched or not fetched[0]:
        return None
    vector = fetched[0]
    _store_cached_vector(QUERY_CACHE_PREFIX, model, digest, vector, ttl=_query_cache_ttl())
    return vector
