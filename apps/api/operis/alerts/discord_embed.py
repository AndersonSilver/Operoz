...
from operis.alerts.types import AlertContext
from operis.discord_integration.services.discord_formatting import (
    OPERIS_EMBED_COLOR,
    OPERIS_EMBED_COLOR_DANGER,
    OPERIS_EMBED_COLOR_WARNING,
    build_branded_embed,
)
from operis.discord_integration.services.text_utils import truncate_for_discord
from operis.utils.support_criticality import criticality_label
...
def _is_brand_workspace(workspace) -> bool:
    ...
    return normalized in ("operoz", "operis", "operozos") or slug in ("operoz", "operis")


def _support_color(criticality: str | None) -> int:
    if criticality in ("p0", "p1"):
        return OPERIS_EMBED_COLOR_DANGER
    if criticality in ("p2",):
        return OPERIS_EMBED_COLOR_WARNING
    return OPERIS_EMBED_COLOR
