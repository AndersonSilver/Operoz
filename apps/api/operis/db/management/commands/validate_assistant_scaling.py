...
from operis.assistant.observability import collect_assistant_metrics, evaluate_assistant_alerts, render_prometheus_metrics
from operis.devops.celery_queue_monitor import default_monitored_queues, get_queue_depths, queues_exceeding_threshold, default_alert_threshold
...
class Command(BaseCommand):
    ...
    def _pytest_env(self) -> dict[str, str]:
        return {
            **dict(__import__("os").environ),
            "DJANGO_SETTINGS_MODULE": "operis.settings.test",
        }
    ...
    def _run_pytest_suite(self, *, deep: bool) -> list[str]:
        ...
        targets = [
            "operis/tests/unit/assistant",
            "operis/tests/unit/devops/test_celery_queue_monitor.py",
            "operis/tests/contract/app/test_assistant_app.py",
        ]
        if deep:
            targets.append("operis/tests/unit/docs/test_assistant_documentation.py")
    ...
    def _run_phase_e2e_suite(self) -> list[str]:
        ...
        for phase_num, class_name in phase_classes:
            ...
            path = f"operis/tests/unit/assistant/test_scaling_phases_e2e.py::{class_name}"
        ...
        if repo is None:
            self.stdout.write(
                self.style.WARNING(
                    ...
                    "executar localmente: cd Operis/apps/web && pnpm check:types"
                )
            )
            return []
    ...
    def _check_stale_jobs(self) -> list[str]:
        ...
        from operis.assistant.chat_jobs import reclaim_stale_assistant_jobs
    ...
    def _run_load_validation(self, vus: int) -> list[str]:
        ...
        result = subprocess.run(
            [
                ...
                "-m",
                "pytest",
                "operis/tests/unit/assistant/test_scaling_validation.py::TestScalingValidation::test_concurrent_chat_load",
                "-q",
                "-s",
            ],
            capture_output=True,
