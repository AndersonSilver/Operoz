...
OPERIS_EMBED_COLOR = 0x4569B3
OPERIS_EMBED_COLOR_SUCCESS = 0x2D8A5E
OPERIS_EMBED_COLOR_WARNING = 0xC47F17
OPERIS_EMBED_COLOR_DANGER = 0xC93C37
...
def _embed_color_for_percent(percent: int) -> int:
    if percent >= 75:
        return OPERIS_EMBED_COLOR_SUCCESS
    if percent >= 35:
...
def build_error_reply(message: str) -> DiscordReply:
    embed = _base_embed(
        ...
        description=message,
        color=OPERIS_EMBED_COLOR_DANGER,
    )
