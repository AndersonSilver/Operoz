...
from operis.assistant.page_content import collect_html_document_embeds_from_content
from operis.db.models import Page, PageReviewInvite, PageReviewSession, Project, ProjectMember, ProjectPage
...
class TestInjectGuestPrdReviewSdk:
    def test_sdk_injection_does_not_break_on_regex_escapes_in_bundle(self):
        ...
        with patch("operis.utils.page_review_guest.read_prd_review_sdk_bundle") as mock_sdk:
            mock_sdk.return_value = "function demo() { return /\\d+/.test('1'); }"
            injected = __import__("operis.utils.page_review_guest", fromlist=["inject_guest_prd_review_sdk"]).inject_guest_prd_review_sdk(
                html,
                invite=invite,
                page_name="PRD",
                read_only=False,
...
class TestBuildGuestPrdReviewPayload:
    ...
    @patch("operis.assistant.page_content.read_html_document_asset_html")
