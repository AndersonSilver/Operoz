...
def test_gemini_default_base_url_when_not_configured():
    with patch(
        "operis.assistant.llm.config.get_configuration_value",
        return_value=(None,),
    ):
...
def test_gemini_model_alias_maps_deprecated_ids():
    with patch("operis.assistant.llm.config.get_api_key", return_value="sk-test"), patch(
        "operis.assistant.llm.config.list_api_keys",
        return_value=[],
    ), patch(
        "operis.assistant.llm.config.get_configuration_value",
        side_effect=[
