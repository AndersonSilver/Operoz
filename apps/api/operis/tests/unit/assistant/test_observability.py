...
from operis.assistant.observability import (
...
from operis.devops.celery_queue_monitor import queue_alert_threshold, queues_exceeding_threshold


@pytest.mark.unit
class TestAssistantObservability:
    @patch("operis.assistant.observability.count_stale_assistant_jobs", return_value=0)
    @patch("operis.assistant.observability.get_queue_depths", return_value={"assistant-chat": 12})
    @patch("operis.assistant.observability._count_active_chats", return_value=3)
    @patch("operis.assistant.observability.llm_semaphore_available", return_value=37)
    @patch("operis.assistant.observability.llm_slots_in_use", return_value=3)
    @patch("operis.assistant.observability._rag_cache_hit_ratio", return_value=0.75)
    @patch("operis.assistant.observability._global_latency_p95_ms", return_value=2100)
    @patch("operis.assistant.observability._chat_error_rate", return_value=0.01)
    ...
    @patch("operis.assistant.observability.redis_instance")
