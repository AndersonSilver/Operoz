...
from operis.assistant.chat_jobs import _stream_key, publish_job_event
from operis.assistant.llm.degraded_mode import should_use_degraded_mode
from operis.assistant.observability import render_prometheus_metrics
from operis.assistant.service import AssistantServiceError, iter_chat_events
from operis.bgtasks.assistant_chat_task import run_assistant_chat_job_task
from operis.db.models import AssistantChatJob, AssistantSession, User, WorkspaceMember
...
def _require_monorepo_root() -> Path:
    root = _repo_root_optional()
    if root is None:
        pytest.skip("Monorepo Operis/ not mounted (skip infra file checks in api-only container)")
    return root
...
class TestPhase1Streaming:
    @patch("operis.assistant.service.release_active_chat")
    @patch("operis.assistant.service.acquire_active_chat", return_value=True)
    @patch("operis.assistant.service.release_llm_slot")
    @patch("operis.assistant.service.try_acquire_llm_slot", return_value=True)
    @patch("operis.assistant.service.check_assistant_rate_limit", return_value=(True, "ok", 0))
    @patch("operis.assistant.service.record_assistant_message")
    @patch("operis.assistant.service.hybrid_retrieve", return_value=[])
    @patch("operis.assistant.service.stream_chat_completion")
    ...
    @patch("operis.app.views.assistant.sessions.iter_chat_events")
...
class TestPhase2Infrastructure:
    def test_docker_compose_defines_api_chat_and_pgbouncer(self):
        ...
        assert "operis-pgbouncer:" in compose
...
class TestPhase3AsyncQueue:
    @patch("operis.bgtasks.assistant_chat_task.iter_chat_events")
    @patch("operis.bgtasks.assistant_chat_task.wait_for_llm_resources", return_value=True)
    @patch("operis.bgtasks.assistant_chat_task.release_llm_slot")
    ...
    @patch("operis.app.views.assistant.sessions.enqueue_chat_job_safe")
...
class TestPhase4RagOptimization:
    @patch("operis.assistant.service.hybrid_retrieve")
    @patch("operis.assistant.service.release_active_chat")
    @patch("operis.assistant.service.acquire_active_chat", return_value=True)
    @patch("operis.assistant.service.release_llm_slot")
    @patch("operis.assistant.service.try_acquire_llm_slot", return_value=True)
    ...
    def test_rag_context_injected_before_llm(
        ...
        from operis.assistant.retrieval import RetrievedChunk
    ...
    def test_long_thread_schedules_async_summary(self, mock_task, create_user, workspace):
        from operis.assistant.thread_summarization import schedule_session_summarization
        from operis.db.models import AssistantMessage
...
class TestPhase5ConcurrencyProtection:
    ...
    @patch("operis.assistant.service.acquire_active_chat", return_value=True)
    @patch("operis.assistant.service.release_llm_slot")
    @patch("operis.assistant.service.try_acquire_llm_slot", return_value=False)
    @patch("operis.assistant.service.check_assistant_rate_limit", return_value=(True, "ok", 0))
    ...
    @patch("operis.assistant.llm.concurrency.llm_slots_in_use", return_value=99)
    def test_degraded_mode_when_queue_saturated(self, _slots, workspace):
        assert should_use_degraded_mode(workspace) is True

    @patch("operis.assistant.service.check_assistant_rate_limit", return_value=(False, "concurrent_rate_limit", 30))
    def test_concurrent_rate_limit_raises_with_retry(self, _rate, create_user, workspace):
        from operis.assistant.chat_jobs import enqueue_chat_job
    ...
    @patch("operis.bgtasks.assistant_chat_task.wait_for_llm_resources", return_value=False)
...
class TestPhase6FrontendContract:
    ...
    @patch("operis.app.views.assistant.sessions.iter_job_events")
...
class TestPhase7Observability:
    @patch("operis.assistant.observability.collect_assistant_metrics")
...
class TestPhase8Governance:
    GOVERNANCE_DOCS = [
        "operis-assistant-adr-004-chat-scaling.md",
    ...
    def test_adr_004_documents_key_decisions(self):
        content = (_docs_root() / "operis-assistant-adr-004-chat-scaling.md").read_text(encoding="utf-8")
    ...
    def test_adrs_index_links_adr_004(self):
        ...
        assert "operis-assistant-adr-004-chat-scaling.md" in content
