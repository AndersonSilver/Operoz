...
from operis.db.models import AssistantChatJob, AssistantMessage, AssistantSession, WorkspaceMember


def _collect_events(job_id: str, *, timeout_seconds: float = 2, idle_seconds: float = 0.2) -> list[dict]:
    with patch("operis.assistant.chat_jobs._stream_idle_timeout_seconds", return_value=idle_seconds):
        return list(iter_job_events(job_id, timeout_seconds=timeout_seconds))


@pytest.mark.django_db
class TestJobStreamResilience:
    @patch("operis.assistant.chat_jobs.redis_instance")
...
class TestStaleJobReaper:
    ...
    @patch("operis.assistant.chat_jobs.release_active_chat")
    @patch("operis.assistant.chat_jobs.release_llm_slot")
    ...
    def test_publish_roundtrip_still_works(self):
        ...
        with patch("operis.assistant.chat_jobs._stream_idle_timeout_seconds", return_value=90):
