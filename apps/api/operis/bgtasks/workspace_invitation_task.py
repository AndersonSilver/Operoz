...
# Module imports
from operis.db.models import User, Workspace, WorkspaceMemberInvite
...
def workspace_invitation(email, workspace_id, token, current_site, inviter):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} convidou você para {workspace.name} no Tech4Humans"  # noqa: E501
        ...
        msg.send()
        logging.getLogger("operis.worker").info("Email sent successfully")
        return
