...
export const BoardCalendarLayoutRoot = observer(function BoardCalendarLayoutRoot() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      clear();
      await fetchFilters(workspaceSlug, boardSlug, { preferredLayout: EIssueLayoutTypes.CALENDAR });
  ...
  }, [boardSlug, clear, fetchFilters, workspaceSlug]);

  const filtersReady = Boolean(workItemFilters?.displayFilters);
  const calendarLayoutReady = workItemFilters?.displayFilters?.layout === EIssueLayoutTypes.CALENDAR;
  const showLoader = isBootstrapping || !filtersReady || !calendarLayoutReady;
        ...
        {({ filter: boardWorkItemsFilter }) => (
          <BoardHubContentPanel>
            ...
            <div className="min-h-0 flex-1">
              <BoardCalendarRoot viewId={boardSlug} />
            </div>
            <IssuePeekOverview />
          </BoardHubContentPanel>
        )}
