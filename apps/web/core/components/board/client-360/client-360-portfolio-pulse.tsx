            ...
            <ul className="divide-y divide-subtle rounded-lg border border-subtle/80">
              {overdueTop.map((c) => (
                <li key={c.project_id} className="flex items-center justify-between gap-3 px-3 py-2.5">
                  ...
                  <span className="shrink-0 text-12 font-semibold text-primary tabular-nums">{c.issues.overdue}</span>
                </li>
              ))}
            </ul>
          ) : (
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid gap-5 xl:grid-cols-2 xl:items-stretch">
        {sidebar ? (
          <div className="order-2 flex min-h-0 min-w-0 flex-col xl:order-1 [&>*]:h-full">{sidebar}</div>
        ) : null}

        <div className={cn("order-1 flex min-w-0 flex-col gap-4", sidebar && "xl:order-2")}>
          {statsTiles ? (
            <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-3 sm:items-stretch">{statsTiles}</div>
          ) : null}
