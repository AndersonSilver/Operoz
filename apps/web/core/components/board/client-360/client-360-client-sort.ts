...
export function sortClient360Clients(
  ...
  return [...clients].sort((a, b) => {
    ...
    switch (column) {
      ...
      case "intake":
        cmp = (a.intake?.pending ?? 0) - (b.intake?.pending ?? 0);

        break;

      case "blockers":
        cmp = (a.blockers?.count ?? 0) - (b.blockers?.count ?? 0);

        break;

      case "throughput":
        cmp = (a.delivery?.throughput ?? 0) - (b.delivery?.throughput ?? 0);

        break;

      case "utilization":
        cmp = (a.finops?.utilization?.pct ?? -1) - (b.finops?.utilization?.pct ?? -1);

        break;

      case "margin":
        cmp = (a.finops?.margin_pct ?? -1) - (b.finops?.margin_pct ?? -1);
