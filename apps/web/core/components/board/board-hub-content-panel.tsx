...
export function BoardHubContentPanel({ children, className }: Props) {
  ...
  return (
    <div className={cn("box-border flex h-full min-h-0 w-full flex-col", hasBackground && "p-4 md:p-5")}>
      <div
        className={cn(
          "box-border flex min-h-0 flex-1 flex-col overflow-hidden",
          hasBackground ? BOARD_HUB_CONTENT_PANEL_IMMERSIVE : "border-t border-subtle bg-surface-1",
          className
        )}
      >
        {children}
