...
import { Circle, Link2, Target } from "lucide-react";
...
import { boardDemandFormService, DEFAULT_BOARD_DEMAND_FIELDS } from "@/services/board/board-intake-form.service";
...
function BoardDemandFormEditorModal(props: EditorProps) {
  ...
  const [fields, setFields] = useState<TIntakeFormField[]>(form?.fields ?? DEFAULT_BOARD_DEMAND_FIELDS ?? []);
  ...
  const removeField = useCallback(
    (fieldId: string) => {
      ...
      if (target?.field_type === "circle") {
        setToast({
          type: TOAST_TYPE.ERROR,
          title: t("toast.error"),
          message: t("boards.settings.demand_forms.circle_field_locked"),
        });
        return;
      }
  ...
  const handleSave = async () => {
    ...
    if (!fields.some((field) => field.field_type === "circle")) {
      setToast({
        type: TOAST_TYPE.ERROR,
        title: t("toast.error"),
        message: t("boards.settings.demand_forms.circle_field_locked"),
    ...
    try {
      ...
      if (form?.id) {
        await boardDemandFormService.update(workspaceSlug, boardSlug, form.id, payload);
      } else {
        await boardDemandFormService.create(workspaceSlug, boardSlug, payload);
      }
      setToast({
        type: TOAST_TYPE.SUCCESS,
        title: t("toast.success"),
        message: t("boards.settings.demand_forms.saved"),
      });
      await onSaved();
    } catch {
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-subtle bg-surface-1 px-6 py-4">
          <div>
            <h2 className="text-16 font-semibold text-primary">
              {form ? form.name : t("boards.settings.demand_forms.create")}
            </h2>
            <p className="mt-0.5 text-12 text-tertiary">{t("boards.settings.demand_forms.subtitle")}</p>
          </div>
              ...
              <div className="grid gap-4 rounded-xl border border-subtle bg-surface-1 p-4">
                ...
                <div className="space-y-2">
                  <label className="text-11 font-semibold text-secondary uppercase">
                    {t("boards.settings.demand_forms.theme_label")}
                  </label>
                  <select
...
export function BoardDemandFormsSettings(props: { workspaceSlug: string; board: IBoard }) {
  ...
  const {
    ...
    isLoading,
    error,
  } = useSWR(`BOARD_DEMAND_FORMS_${workspaceSlug}_${board.slug}`, () =>
    boardDemandFormService.list(workspaceSlug, board.slug)
  );
  ...
  const handleCreate = useCallback(async () => {
    setCreating(true);
    try {
      const created = await boardDemandFormService.create(workspaceSlug, board.slug, {
        name: t("boards.settings.demand_forms.default_name"),
        header_title: t("boards.settings.demand_forms.default_header"),
        fields: DEFAULT_BOARD_DEMAND_FIELDS,
        theme: "default",
        defaults: {},
        submit_message: t("boards.settings.demand_forms.default_submit_message"),
      });
      await mutate();
      ...
      setToast({
        type: TOAST_TYPE.SUCCESS,
        title: t("toast.success"),
        message: t("boards.settings.demand_forms.created"),
      });
    } catch {
      setToast({ type: TOAST_TYPE.ERROR, title: t("toast.error"), message: t("something_went_wrong") });
    } finally {
  ...
  const handleDelete = useCallback(
    async (form: TBoardIntakeForm) => {
      if (!window.confirm(t("boards.settings.demand_forms.delete_confirm"))) return;
      try {
        await boardDemandFormService.remove(workspaceSlug, board.slug, form.id);
        await mutate();
        setToast({
          type: TOAST_TYPE.SUCCESS,
          title: t("toast.success"),
          message: t("boards.settings.demand_forms.deleted"),
        });
      } catch {
        setToast({ type: TOAST_TYPE.ERROR, title: t("toast.error"), message: t("something_went_wrong") });
      }
  ...
  return (
    <div className="space-y-6">
      <SupportSettingsHero
        icon={Circle}
        title={t("boards.settings.demand_forms.title")}
        description={t("boards.settings.demand_forms.subtitle")}
        createLabel={t("boards.settings.demand_forms.create")}
        creating={creating}
        onCreate={() => void handleCreate()}
        highlights={[
          { label: t("boards.settings.demand_forms.highlight_public"), icon: Link2, tone: "accent" },
          { label: t("boards.settings.demand_forms.highlight_circle"), icon: Target, tone: "success" },
        ]}
      />
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-danger-subtle bg-danger-subtle px-5 py-4 text-13 text-danger-primary">
          {t("boards.settings.demand_forms.load_error")}
        </div>
      ) : (
        <IntakeFormGrid<TBoardIntakeForm>
          forms={sortedForms}
          buildUrl={buildPublicFormUrl}
          i18nPrefix="boards.settings.demand_forms"
          creating={creating}
          onCreate={() => void handleCreate()}
