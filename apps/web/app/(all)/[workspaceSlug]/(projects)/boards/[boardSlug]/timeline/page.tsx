...
export default function BoardTimelinePage(_props: Route.ComponentProps) {
  ...
  if (isBoardLoading && !board) {
    return (
      <>
        <PageHead title={t("boards.timeline_title")} />
  ...
  const pageTitle = currentWorkspace?.name
    ? `${currentWorkspace.name} - ${board.name} - ${t("boards.tab_timeline")}`
    : undefined;

  return (
    <>
      <PageHead title={pageTitle} />
      <BoardTimelineLayoutRoot />
    </>
  );
}
