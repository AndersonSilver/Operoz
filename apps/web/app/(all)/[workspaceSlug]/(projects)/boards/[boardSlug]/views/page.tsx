...
import { BoardViewsLayoutRoot } from "@/components/board/board-views-layout-root";
import { PageHead } from "@/components/core/page-title";
import { LogoSpinner } from "@/components/common/logo-spinner";
import { useWorkspace } from "@/hooks/store/use-workspace";
import type { Route } from "./+types/page";

export default function BoardViewsPage(_props: Route.ComponentProps) {
  ...
  if (isBoardLoading && !board) {
    return (
      <>
        <PageHead title={t("boards.views_title")} />
  ...
  const pageTitle = currentWorkspace?.name
    ? `${currentWorkspace.name} - ${board.name} - ${t("boards.tab_board")}`
    : undefined;

  return (
    <>
      <PageHead title={pageTitle} />
      <BoardViewsLayoutRoot />
    </>
  );
}
