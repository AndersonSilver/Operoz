...
export const WorkspaceIntegrationConfigModal = observer(function WorkspaceIntegrationConfigModal(props: Props) {
  const { workspace, isOpen, onClose } = props;
  const { t } = useTranslation();
  const { patchWorkspaceIntegrationFlags, loader: workspaceLoader } = useWorkspace();
  const saving = workspaceLoader === "mutation";

  const patchFlag = async (payload: Parameters<typeof patchWorkspaceIntegrationFlags>[1]) => {
    if (!workspace) return;

    const promise = patchWorkspaceIntegrationFlags(workspace.id, payload);
    setPromiseToast(promise, {
      loading: t("god_mode.common.config_saving"),
      success: {
        title: t("god_mode.common.success"),
        message: () => t("god_mode.pages.workspace.integrations.save_success"),
      },
              ...
              <Dialog.Panel className="relative w-full max-w-lg transform overflow-hidden rounded-xl border border-subtle bg-surface-1 shadow-raised-200 transition-all">
                <div className="flex items-start gap-3 border-b border-subtle px-5 py-4">
                  ...
                  <div className="min-w-0">
                    <Dialog.Title className="text-16 font-semibold text-primary">
                      {t("god_mode.pages.workspace.integrations.modal_title")}
                    </Dialog.Title>
                    <p className="mt-1 text-13 leading-relaxed text-secondary">
                      {workspace
                        ? t("god_mode.pages.workspace.integrations.modal_description", { name: workspace.name })
                        : t("god_mode.pages.workspace.integrations.modal_hint")}
                    </p>
                  </div>
                </div>
                ...
                <div className="max-h-[min(70vh,32rem)] space-y-2 overflow-y-auto p-5">
                  <p className="text-12 leading-relaxed text-tertiary">
                    {t("god_mode.pages.workspace.integrations.lead")}
                  </p>
                  {workspace
                    ? WORKSPACE_INTEGRATION_FLAGS.map((flag) => (
                        <AdminToggleCard
                          key={flag.key}
