...
const PANEL_BASE_CLASS = "rounded-md border-[0.5px] border-subtle bg-surface-1 shadow-raised-200";
...
export function IntakePublicEditorToolbar(props: Props) {
  const { editorRef, editorReady = false } = props;
  ...
  const updateActiveStates = useCallback(() => {
    ...
    FORM_RICH_TEXT_TOOLBAR_GROUPS.forEach((group) => {
  ...
  const activeTypography =
    FORM_RICH_TEXT_TOOLBAR_GROUPS.find((g) => g.key === "typography")?.items.find(
      (item) => activeStates[item.renderKey]
    ) ?? FORM_RICH_TEXT_TOOLBAR_GROUPS.find((g) => g.key === "typography")?.items[0];
    ...
    >
      <div className="flex w-full items-center gap-2 overflow-x-auto overflow-y-visible">
        <div className="flex min-w-0 flex-1 items-stretch gap-0.5">
          ...
          >
            {FORM_RICH_TEXT_TOOLBAR_GROUPS.find((g) => g.key === "typography")?.items.map((item) => (
              <button
                key={item.renderKey}
                type="button"
                className={cn(
          ...
          {FORM_RICH_TEXT_TOOLBAR_GROUPS.filter((g) => g.key !== "typography").map((group, groupIndex) => (
            <div
              key={group.key}
