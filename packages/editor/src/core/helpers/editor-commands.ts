...
import type { InsertHtmlDocumentEmbedProps } from "@/extensions/html-document/types";
// helpers
...
export const insertHtmlDocument = ({
  editor,
  event,
  pos,
  file,
  ...
  const opts: InsertHtmlDocumentEmbedProps = { event };
  if (pos !== undefined && pos !== null) opts.pos = pos;
  if (file) opts.file = file;
  return editor?.chain().focus().insertHtmlDocumentEmbed(opts).run();
};
