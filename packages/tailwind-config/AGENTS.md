...
- Surfaces are **siblings**, not nested (in the same operoz)
- Each surface should use its corresponding layer for nested elements
- Surfaces provide the base for stacking layers

**Exception - Different Operozs**:

- Modals, overlays, and popovers exist on a **different operoz** (different z-index/stacking context)
...
// ❌ Wrong: Surface nested in surface (same operoz)
<div className="bg-surface-1">
  <div className="bg-surface-2">
...
// ✅ Correct: Modal on different operoz
<div className="bg-canvas">
  {/* Main page content */}
  <div className="bg-surface-1">
    Page content
  ...
  {/* Modal overlay - different operoz */}
...
```tsx
// ✅ Correct: Modal structure (different operoz exception)
function PageWithModal() {
  return (
    <div className="bg-surface-1">
      ...
      {/* Modal - different operoz, can use surface even with surface below */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50">
          <div className="bg-backdrop fixed inset-0" />
          <div className="bg-surface-1 rounded-lg shadow-lg p-6">
...
OR
Is this a modal/overlay on a different operoz (z-index)?
...
### ❌ Mistake 2: Nested Surfaces (Same Operoz)

```tsx
// ❌ Wrong: Nested surfaces in same operoz
<div className="bg-surface-1">
  <div className="bg-surface-2">
    Nested surface
  </div>
...
// ✅ Correct: Exception - Modal on different operoz
<div className="bg-canvas">
  <div className="bg-surface-1">Page content</div>
  <div className="fixed inset-0 z-50">
    <div className="bg-surface-1">Modal (different operoz)</div>
  </div>
</div>
```
...
2. **Use Surfaces for Pages and Top-Level Containers**: Surfaces are siblings, not nested (except modals/overlays on different operozs)
...
- [ ] Surfaces are siblings, not nested (except modals/overlays on different operozs)
