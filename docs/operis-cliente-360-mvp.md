# Cliente 360 — MVP (Operis)

**Objetivo:** vista executiva no **board** para acompanhar cada cliente (projeto) sem abrir dezenas de ecrãs.

**Modelagem assumida (Modelo A):** 1 projeto Operis = 1 cliente; módulos = frentes de entrega.
...
3. Widget no Resumo do board («3 clientes sem report»)
4. Export CSV da matriz cliente × semana
5. Integração automação (lembrete sexta-feira)
6. Histórico multi-semana (tendência em linha)
