# Operis — Automação interna (MVP) — Plano e questionário

Documento vivo para definir o **motor de automação nativo do Operis** (antes Kortex / fork Plane Tech4Humans).  
...
| **Estado** | **Spec técnica publicada** — ver [operis-automacao-mvp-spec.md](./operis-automacao-mvp-spec.md) |
...
| 9 | **n8n / Node-RED** | **Não** no deploy padrão — só motor nativo Operis |
...
| 13 | **Filtros «Se»** | **Catálogo dinâmico** — expor todos os filtros que o Operis suportar |
...
Permitir que equipas configurem **regras dentro do Operis** para reagir a eventos (issues, status report, etc.) e executar **ações** (alterar campos, notificar, webhooks, IA) **sem deploy de código** e **sem depender só de ferramentas externas**.
...
> «Quando X acontecer no Operis, se Y for verdade, fazer Z» — com histórico de execução e permissões por workspace/projeto.

---

## 3. Contexto: o que o Operis já tem
...
- Plugin **Atlassian** (Jira / Confluence): backlog, status report, triagem — fluxos de *conteúdo* entre sistemas.
...
**Alvo do Operis (MVP):** mesmo modelo mental + **canvas visual no MVP** (React Flow ou equivalente), com catálogo dinâmico de nós (gatilho / filtro / ação).

---

## 5. Decisão estratégica: motor próprio vs ferramentas externas

| Abordagem | Quando usar | No Operis |
|----------|-------------|-----------|
...
| **n8n** | Cliente com ERP/Slack complexo: webhook do Operis → fluxo n8n |
...
- Sincronização bidirecional completa Jira ↔ Operis (projeto à parte; ação pontual Jira pode entrar no catálogo).
...
MV? Automação Fase 1 (este documento)  ← início após PRD estável
        ...
        └── MV6 Rebranding Operis (ex-Kortex)
```
...
**3.2** Fuso horário → *a definir na spec* (quando implementar cron)
...
**5.2** n8n no deploy → **Não** — só motor nativo Operis
...
1. ~~Escrever spec técnica~~ → [operis-automacao-mvp-spec.md](./operis-automacao-mvp-spec.md).
