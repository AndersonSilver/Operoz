...
**Decisão:** Cookie `operis_sse_route` para afinidade ao mesmo `api-chat` durante SSE.

**Motivo:** reconexão SSE e streams longos sem saltar entre réplicas stateless.

## Consequências

- Novos serviços: `api-chat`, `assistant-chat-worker`, `operis-pgbouncer`
...
| Componente      | Caminho                                            |
| --------------- | -------------------------------------------------- |
| Pipeline chat   | `operis/assistant/service.py`                      |
| Jobs async      | `operis/assistant/chat_jobs.py`                    |
| Worker          | `operis/bgtasks/assistant_chat_task.py`            |
| Semáforo / fila | `operis/assistant/llm/concurrency.py`              |
