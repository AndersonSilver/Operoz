# Operis — Espaço de trabalho (Workspace): referência Plane + MVP de implementação

Documento de produto e engenharia baseado na documentação oficial do [Plane](https://docs.plane.so/). Objetivo: definir um **MVP priorizado** para a secção **Gestão do espaço de trabalho** no Operis (fork Plane + camada Board Tech4Humans).

> **Catálogo completo (todas as secções da sidebar):** [operis-plane-catalogo-completo-docs.md](./operis-plane-catalogo-completo-docs.md) — inclui Gerenciamento de projetos, Itens de trabalho, Planejamento, Vistas, Conhecimento, Gestão avançada, Comunicação, Admissão, Integrações, Import/Export e IA.
...
No Operis, o mesmo conceito existe no modelo `Workspace` e na rota `/{workspaceSlug}/…`. A hierarquia **Tech4Humans** acrescenta **Board** entre workspace e projeto:
...
Legenda de prioridade sugerida para o Operis:

| Tag       | Significado                                            |
| --------- | ------------------------------------------------------ |
| **MVP**   | Entregar na primeira onda “workspace Operis”           |
| **MVP+**  | Segunda onda, logo após MVP estável                    |
| **Stock** | Já existe no fork (herdado do Plane); validar UX/copy  |
| **Gap**   | Plane tem; Operis precisa alinhar ou não tem           |
| **CE**    | Plane Commercial / plano pago — fora de escopo inicial |
| **N/A**   | Não aplicável ao Operis self-hosted / produto          |

### 2.1 Ciclo de vida do workspace

| Funcionalidade             | Descrição (Plane)                                    | Tag Operis                                                             |
| -------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------- |
...
| Transferir propriedade     | Só **Owner**                                         | **Gap** — Operis usa `owner` FK + Admin(20), sem papel Owner explícito |
...
| Recurso workspace        | Descrição breve                             | Tag Operis                           |
| ------------------------ | ------------------------------------------- | ------------------------------------ |
...
| Teamspaces               | Agrupamento tipo “time” com link a projetos | **CE** — Operis usa **Board** custom |
...
| Funcionalidade              | Detalhe Plane                                             | Tag Operis                     |
| --------------------------- | --------------------------------------------------------- | ------------------------------ |
...
| Papel workspace | Quem é                                                        | Operis hoje (`ROLE_CHOICES`)                                                |
| --------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------- |
...
| RBAC vs GAC                                       | Papéis fixos vs papéis custom + permission schemes (**Enterprise**)   | Operis MVP: **RBAC** apenas |
...
| Funcionalidade                        | Detalhe                                                                        | Tag Operis                     |
| ------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------ |
...
| Funcionalidade                | Detalhe                                                         | Modelo Operis                                           |
| ----------------------------- | --------------------------------------------------------------- | ------------------------------------------------------- |
...
| Secção settings | Conteúdo típico                              | Tag Operis                    |
| --------------- | -------------------------------------------- | ----------------------------- |
...
| MCP Server oficial  | Operis tem [mcp-server](../mcp-server/README.md) — alinhar escopo |

---

## 3. Estado atual no Operis (código)

### 3.1 Modelo de dados (`apps/api/operis/db/models/workspace.py`)
...
### 3.2 Permissões (`apps/api/operis/utils/permissions/workspace.py`)
...
### 3.4 Diferenças Plane × Operis (resumo)

| Tópico        | Plane (doc 2025+)                   | Operis                                      |
| ------------- | ----------------------------------- | ------------------------------------------- |
...
## 4. MVP Operis — “Workspace sólido”

### 4.1 Objetivo do MVP

Entregar uma experiência de **espaço de trabalho** previsível para equipas Tech4Humans/Operis:
...
**Stock** — validar regressões após rebranding Operis.
...
| E3  | SMTP documentado para convites | `Operis/deployments/…` + troubleshooting      |

#### Épico F — Alinhamento Tech4Humans (**MVP**, específico Operis)
...
## 7. Glossário PT ↔ Plane ↔ Operis

| Português (UI Operis) | Plane (EN)                    | Entidade código          |
| --------------------- | ----------------------------- | ------------------------ |
...
| [operis-mcp.md](./operis-mcp.md)                                                             | MCP empresa/workspace      |
...
O **Operis já herda a maior parte** do modelo Plane via fork. O MVP proposto **não replica o Plane Commercial inteiro**; foca em:
