# Operis MCP — implantação para empresa (~150 utilizadores Cursor)

Objetivo: **ninguém clona o monorepo Operis**. A equipa usa o **Operis hospedado** no browser; no Cursor só configura o **MCP** para criar tarefas, boards, cards, páginas (PRD), etc.
...
1. Entra no Operis → **Definições → API tokens** → cria token **pessoal** (não partilhar).
...
Quando o serviço HTTP estiver ativo no vosso deploy (`npm run start:http` / imagem `operis-mcp`):
...
IT instala **uma vez** o binário MCP (imagem Docker ou artefacto em rede interna). O colaborador **não** clona o Operis:
...
```json
{
  "mcpServers": {
    "operis": {
      "command": "node",
      "args": ["/usr/lib/operis/mcp-server/dist/index.js"],
      "env": {
        "OPERIS_API_BASE_URL": "https://operis.sua-empresa.com",
        "OPERIS_API_KEY": "token-pessoal"
      }
    }
  }
}
...
- **Boards, páginas (PRD), Cliente 360** usam a API **app**, que hoje pede **cookie de sessão** (`operis_sign_in` ou `OPERIS_SESSION_COOKIE`). Num MCP **partilhado** (um servidor para todos), login por password na ferramenta **não** é aceitável.

**Roadmap recomendado para a tua empresa:**

1. **Fase 1 (já):** token por utilizador → tarefas, projetos, comentários, pesquisa.
2. **Fase 2:** MCP HTTP centralizado com token no header (deploy `operis-mcp`).
...
2. Validar prompts típicos: “cria tarefa no projeto X”, “move para Em progresso”, “comenta na OPERIS-123”.
