...
O pacote `mcp-server/` expõe o Operoz a agentes de IA (Cursor, Claude Desktop, etc.) via [Model Context Protocol](https://modelcontextprotocol.io).

## Localização

`/Operoz/mcp-server/` (relativo à raiz do monorepo).

## Perfil de ferramentas (Cursor)

Por defeito o MCP usa **`OPEROZ_MCP_PROFILE=agent`** — **7 tools** expostas, registo interno com **676 operações**:

| Tool                                               | Função                                              |
| -------------------------------------------------- | --------------------------------------------------- |
| `operoz_discover`                                  | Encontra operações por intenção (`query`, `domain`) |
| `operoz_execute`                                   | Executa pelo `name` devolvido pelo discover         |
| `operoz_get_capabilities`                          | Mapa de domínios                                    |
| `operoz_sign_in`                                   | Sessão para API app                                 |
| `operoz_api_v1_request` / `operoz_api_app_request` | Escape hatch genérico                               |
| `operoz_get_openapi_schema`                        | Schema OpenAPI                                      |

Fluxo: `operoz_discover` → `operoz_execute`. Perfil `full` (`OPEROZ_MCP_PROFILE=full`) expõe 1 tool por endpoint (legado/debug).
...
Use `operoz_discover` com `query` e `domain` (ex.: `"list pages"`, `"boards"`) para encontrar a operação correcta antes de `operoz_execute`.
...
- **Cursor / CI / integrações:** MCP perfil `agent` (`operoz_discover` → `operoz_execute`)
...
- Upload binário de assets pode precisar de `operoz_api_*_request` com corpo adequado.
...
Ver [operoz-mcp-empresa.md](./operoz-mcp-empresa.md) — MCP centralizado, token por pessoa, sem clone do repositório.
