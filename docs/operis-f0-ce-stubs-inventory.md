...
**Descoberta:** no Operis, `@/plane-web/*` aponta para `apps/web/ce/*` (`tsconfig.json`). Não existe pasta `plane-web` separada.
...
### Média — CE / teamspace (Board Operis substitui parte)
...
| Domínio Plane                 | Modelo `apps/api/operis/db/models`                 | Estado F0    |
| ----------------------------- | -------------------------------------------------- | ------------ |
| Board (Operis)                | `board.py`, `board_issue_type.py`, `board_role.py` | ✅ custom    |
...
Ao fechar cada stub, marcar no [operis-gap-tracker.md](./operis-gap-tracker.md) e preferir implementação em `core/` quando for feature Operis-first (Board), mantendo `ce/` só para extensões EE-style.
