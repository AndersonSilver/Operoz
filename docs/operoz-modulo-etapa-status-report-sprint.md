...
O **status** do módulo usa opções **globais** em `@operoz/constants` (`MODULE_STATUS`). Não há campo de **etapa** no modelo `Module` nem coluna correspondente na UI.
...
3. Após aprovação: abrir issues/PRs por fase conforme `operoz-gap-tracker.md`.
