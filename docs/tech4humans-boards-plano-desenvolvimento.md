...
- **Projeto:** `apps/api/plane/db/models/project.py` — `workspace = ForeignKey(WorkSpace)`.
- **Entidades de projeto:** herdam `ProjectBaseModel` / `WorkspaceBaseModel` com `workspace` + `project`.

### 2.2 Modelo `Team` (legado parcial)

- **Existe:** `Team` em `apps/api/plane/db/models/workspace.py` (`name`, `description`, `workspace`, `logo_props`).
...
Implementação: `apps/api/plane/app/views/project/base.py` (`ProjectViewSet`) — filtra por `workspace__slug`, membership (`ProjectMember`, rede pública/privada), `sort_order` via `ProjectUserProperty`.
...
| # | Tarefa | Arquivo / ação |
|---|--------|----------------|
| 1.1 | Criar modelo `Board` | `apps/api/plane/db/models/board.py` (ou `workspace.py`) |
...
| Serializers | `ProjectListSerializer`, `ProjectSerializer`, `ProjectDetailSerializer` — `apps/api/plane/app/serializers/project.py` |

### 7.3 Arquivos a criar

```
apps/api/plane/app/views/board/
  __init__.py
  base.py          # BoardViewSet
  member.py        # opcional
apps/api/plane/app/serializers/board.py
apps/api/plane/app/urls/board.py
```
...
- Atualizar `apps/api/plane/settings/openapi.py` e rotas `plane/api/` se expuser boards a integrações.

### 7.6 Busca workspace

- `apps/api/plane/app/views/search.py` (e equivalentes): facet opcional `board_id`; resultados de projeto com board.
...
Implementação em `apps/api/plane/app/permissions/base.py` + `board.py`.
...
apps/api/plane/app/views/board/             # NOVO
