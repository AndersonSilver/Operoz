...
| **Relacionados** | [operoz-cliente-360-mvp.md](./operoz-cliente-360-mvp.md), [operoz-visao-360-roadmap.md](./operoz-visao-360-roadmap.md), [operoz-intake-sustentacao-split-spec.md](./operoz-intake-sustentacao-split-spec.md) |
...
**Toda entrega de sustentação inclui testes unitários** — backend (`pytest` em `apps/api/operoz/tests/unit/intake/`) e frontend quando houver lógica isolável (hooks, utils). Não mergear permissões ou regras de negócio sem cobertura mínima.
...
| **Separação Intake × Sustentação** | Decisão **A** — coexistem no mesmo projeto; ver [operoz-intake-sustentacao-split-spec.md](./operoz-intake-sustentacao-split-spec.md) |
...
**`IntakeIssue`** (`apps/api/operoz/db/models/intake.py`):
...
| Camada                | Path                                                                                     |
| --------------------- | ---------------------------------------------------------------------------------------- |
| Model board form      | `apps/api/operoz/db/models/board_intake_form.py`                                         |
| Submit público        | `apps/api/operoz/utils/board_intake_submission.py`                                       |
| API intake            | `apps/api/operoz/app/views/intake/base.py`, `apps/api/operoz/api/views/intake.py`        |
...
| Permissão board admin | `apps/api/operoz/app/permissions/board_access.py` (`board.administer`)                   |
...
- [x] 0.2 Backend delete → board admin (`operoz/utils/intake_permissions.py`)
...
- [x] 2.1b Filas configuráveis no board + aceitar com fila + encerrar (Sprint 1 — ver `operoz-sustentacao-filas-spec.md`)
...
- `apps/api/operoz/app/views/intake/base.py` — `IntakeIssueViewSet.destroy`
- `apps/api/operoz/api/views/intake.py` — `IntakeIssueDetailAPIEndpoint.delete`
...
- Assistente intake: `apps/api/operoz/assistant/tools/handlers.py` → `list_intake_pending`
