...
```bash
cd Operis
./scripts/run-f0-smoke.sh          # usa container operis-api-1 se estiver up
./scripts/run-f0-smoke.sh --local  # força execução local (apps/api)
```

Ficheiros:

- `apps/api/operis/tests/smoke/test_operis_f0_smoke.py` — 7 testes F0
- `apps/api/operis/tests/smoke/test_auth_smoke.py` — auth + health

Alternativa dentro do container:

```bash
docker exec -w /code operis-api-1 pip install -q -r requirements/test.txt
docker exec -w /code operis-api-1 python -m pytest operis/tests/smoke/ -m smoke -v
```
...
| D2  | `pytest operis/tests/contract/app/test_workspace_app.py` | ☐   | Contract workspace (F1)                                   |
