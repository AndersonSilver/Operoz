# Operoz — Plataforma Violenta: Arquitetura Claude Code + Assistente Conversacional
...
| **Escopo**       | Motor de automação estilo Claude Code + Chat Operoz (RAG + tools)                                                                                          |
| **Relacionados** | [operoz-automacao-mvp-spec.md](./operoz-automacao-mvp-spec.md), [operoz-mcp.md](./operoz-mcp.md), [operoz-cliente-360-mvp.md](./operoz-cliente-360-mvp.md) |

---

## 1. Visão

Transformar o Operoz de um **board com automações** em uma **plataforma operacional inteligente**:
...
**North Star:** _"Pergunte qualquer coisa sobre sua operação e o Operoz responde com dados, ações e automações — não com alucinação."_
...
| -------------------- | --------------------------------------------------------------- | ------------------------------------------------ |
| Automação por board  | Grafo, catalog, executor, outbox, Celery, cron, e-mail, scripts | `apps/api/operoz/automation/`                    |
| LLM básico           | `ai-assistant` workspace/projeto; Ask Pi no editor de páginas   | `apps/api/operoz/app/views/external/base.py`     |
| MCP externo          | 379 tools para agentes externos (Cursor, Claude Desktop)        | `mcp-server/`                                    |
| Search               | Global + issues + entity search                                 | `apps/api/operoz/app/views/search/`              |
| Documentação         | Pages com rich-text, versões, lock                              | `apps/api/operoz/db/models/page.py`              |
...
## 3. Arquitetura Claude Code → Operoz (mapa completo)
...
MCP servers                          Integration Packs + Operoz Tools API
...
│  Chat Operoz │ Galeria Playbooks │ Canvas Automação │ Cliente 360 + IA      │
└───────────────────────────────────┬─────────────────────────────────────────┘
                                    │
┌───────────────────────────────────▼─────────────────────────────────────────┐
...
```json
{
  "name": "operoz-pack-gestao-operacional",
  "version": "1.0.0",
...
| `action.operoz_tool` | Claude tool use                  | Executar tool interna com permissão do user |
...
## 5. Pilar B — Assistente Operoz (Chat)
...
└──────────────┘     └────────┬────────┘     └──────────────────┘
       ...
       └─────┬──────┘  └─────┬──────┘  └────────────┘
             ...
             ▼               ▼
       ┌─────────────────────────────────────────┐
       │ Operoz API (com permissões do usuário)  │
...
| Aspecto    | `ai-assistant` hoje      | Assistente Operoz (alvo)                         |
| ---------- | ------------------------ | ------------------------------------------------ |
...
- Painel lateral global (atalho `Cmd+K` → "Perguntar ao Operoz")
...
```text
1. Identidade: Assistente Operoz, especialista na operação do board
...
6. Formato: markdown, links internos Operoz
```
...
- Operoz automação: [operoz-automacao-mvp-spec.md](./operoz-automacao-mvp-spec.md)
- Operoz MCP: [operoz-mcp.md](./operoz-mcp.md)
- Cliente 360: [operoz-cliente-360-mvp.md](./operoz-cliente-360-mvp.md)
