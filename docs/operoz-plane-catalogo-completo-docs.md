# Operoz × Plane — Catálogo completo da documentação (sidebar)

> **Resposta direta:** o ficheiro anterior ([operoz-workspace-mvp-plane.md](./operoz-workspace-mvp-plane.md)) cobriu sobretudo **Gestão do espaço de trabalho** e recursos transversais (membros, RBAC, search, home). **Não** percorreu página a página as outras 11 secções da sidebar. Este documento corrige isso: inventário **por secção**, com link oficial, resumo funcional e estado no **Operoz**.
...
| 🔄  | Operoz tem equivalente diferente (ex.: Board vs Teamspace) |

---

## 1. Gestão do espaço de trabalho

| Página doc                            | URL                                                                                         | O que faz                                                   | Operoz                      |
| ------------------------------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | --------------------------- |
...
**MVP Operoz (esta secção):** ver [operoz-workspace-mvp-plane.md](./operoz-workspace-mvp-plane.md) — épicos A–E.
...
**Operoz específico:** projeto = **épico de negócio**; agrupado sob **Board** na sidebar — [tech4humans-boards-plano-desenvolvimento.md](./tech4humans-boards-plano-desenvolvimento.md).
...
| Épicos                                      | [epics](https://docs.plane.so/core-concepts/issues/epics)              | Epic = tipo de item (não aba separada); hierarquia nível 1 | ✅ tipo; no Operoz épico = **Projeto** 🔄 |
...
**Hierarquia Tech4Humans:** Card = `Issue`; Subtarefa = `Issue` com `parent`; não confundir Epic Plane com Projeto Operoz.
...
| Espaços de equipe (Teamspaces) | [teamspaces](https://docs.plane.so/core-concepts/workspaces/teamspaces) | Hub do time: itens/ciclos/views cross-project           | 🔄 **Board** Operoz |
...
**Board Operoz (MVP):** substitui Teamspace para “time → N projetos”; ver docs `tech4humans-boards-*`.
...
**Board hub (Operoz MVP-2):** vistas agregadas Lista/Backlog/Quadro no **Board** — [tech4humans-boards-mvp2-plano.md](./tech4humans-boards-mvp2-plano.md).
...
| Automações custom               | custom automations                                                        | Trigger + condição + ação                            | 🟡 — ver [operoz-automacao-mvp-spec.md](./operoz-automacao-mvp-spec.md) |
...
**Operoz:** flags extra em `Workspace` para email de issues (`issue_notify_*` em `workspace.py`).
...
| Integração          | URL                                                 | O que faz                              | Operoz     |
| ------------------- | --------------------------------------------------- | -------------------------------------- | ---------- |
...
| MCP Server         | developer MCP                               | Agente lê/escreve Plane via MCP                          | 🔄 [operoz-mcp.md](./operoz-mcp.md) + `mcp-server/` |

**Operoz:** priorizar **MCP próprio** em vez de copiar Plane AI cloud.
...
→ Plano mestre com fases F0–F18: **[operoz-paridade-plane-plano-completo.md](./operoz-paridade-plane-plano-completo.md)**
...
| Ficheiro                                                               | Conteúdo                     |
| ---------------------------------------------------------------------- | ---------------------------- |
| [operoz-workspace-mvp-plane.md](./operoz-workspace-mvp-plane.md)       | MVP detalhado só workspace   |
| [jira-vs-plane-comparativo.md](./jira-vs-plane-comparativo.md)         | Decisões Jira × Plane/Operoz |
| [tech4humans-plane-organizacao.md](./tech4humans-plane-organizacao.md) | Hierarquia canónica          |
| [operoz-automacao-mvp-spec.md](./operoz-automacao-mvp-spec.md)         | Automações Operoz            |
